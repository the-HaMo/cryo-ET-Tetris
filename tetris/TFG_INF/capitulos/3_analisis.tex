...
Todas las pruebas, ejecuciones y benchmarks de este proyecto se realizaron en una estación de trabajo de escritorio con las siguientes especificaciones:
...
\begin{itemize}
    ...
    \item \textbf{Almacenamiento}: [TODO: especificar]
    \item \textbf{Sistema operativo}: [TODO: especificar, ej.\ Ubuntu 22.04 LTS]
\end{itemize}

\subsubsection{Entorno virtual y dependencias}

El desarrollo y la ejecución del código se llevaron a cabo dentro de un entorno virtual de Python (\texttt{venv}), garantizando el aislamiento de dependencias respecto al sistema y la reproducibilidad de los experimentos. Para recrear dicho entorno deben ejecutarse los siguientes comandos:
...
El módulo \texttt{PolNet} se instaló directamente desde el código fuente del repositorio, al emplearse una versión modificada del \textit{framework} original. Los formatos de archivo utilizados en el proyecto son dos: \textbf{MRC}, para almacenar volúmenes tridimensionales de densidad electrónica, al ser el estándar de la microscopía electrónica; y \textbf{VTP} (VTK PolyData), para representaciones poligonales de estructuras como membranas y anotaciones geométricas. Para la inspección visual de los resultados se utilizaron \textbf{ParaView} (exploración interactiva de volúmenes 3D y mallas) y \textbf{3dmod} (parte del suite IMOD), para la inspección de cortes 2D de los tomogramas.
...
\begin{enumerate}
    ...
    Este diseño produce un empaquetamiento en dos niveles: cuando la inserción es exitosa, el algoritmo continúa en la misma región (\textit{crecimiento local}), favoreciendo clústeres compactos; cuando falla, se muestrea una nueva semilla aleatoria del espacio global (\textit{reset global}), evitando que el algoritmo quede atrapado en zonas saturadas. El proceso finaliza al acumular $T_{cluster}$ fallos consecutivos para el tipo de proteína en curso, momento en el que el algoritmo pasa al siguiente tipo.
     ...
     \item \textbf{Gestión de umbrales y selección de proteínas}: Para garantizar la consistencia en la detección de colisiones, se establece un umbral de binarización global definido como el 8\% del valor máximo de densidad de la proteína de mayor tamaño (tras una ordenación descendente por ocupación interna del catálogo de entrada). El valor del 8\% es un valor convencional en cryo-ET para separar la densidad proteica real del ruido de fondo en mapas de densidad electrónica. Este umbral se mantiene constante y único para todas las iteraciones del algoritmo: si cada tipo de proteína usara su propio umbral, una macromolécula con umbral bajo podría solaparse geométricamente con zonas ya marcadas como ocupadas por otra de umbral mayor, produciendo inconsistencias en la detección de colisiones.

    El algoritmo prioriza la inserción de las proteínas de mayor tamaño para maximizar el empaquetamiento inicial y reducir la fragmentación temprana del volumen libre. El tamaño se determina mediante la \textit{ocupación interna} de cada proteína, definida como:
    ...
    Si la comprobación falla, la posición correspondiente en el \textit{c-map} se penaliza con $-10^9$, descartándola como candidato, de forma análoga a la penalización aplicada sobre vóxeles de membrana en el Punto~2. El algoritmo continúa evaluando las siguientes posiciones de mayor correlación dentro del mismo mapa, con un máximo de 50 intentos por llamada; este límite equilibra la exhaustividad de la búsqueda local con el coste de iterar sobre una región progresivamente saturada. Si ninguna posición supera todas las comprobaciones, el algoritmo devuelve \texttt{'saturated'}, indicando que no existe espacio viable en la región local analizada.

    \item \textbf{Análisis de rendimiento de la implementación adaptada}: Con todas las modificaciones integradas —pre-carga de membrana, \texttt{pick\_seed} con muestreo uniforme y bucle de crecimiento de clústeres— se procedió a medir empíricamente el tiempo real de cada etapa mediante el script \texttt{profile\_test.py}. Este script instrumenta el bucle principal con temporizadores de alta resolución (\texttt{time.perf\_counter}) y aplica \textit{monkey patching} sobre la función \texttt{\_correlate} para aislar el tiempo de la correlación FFT del resto del método \texttt{insert\_molecule\_3d}. El tiempo de validación e inserción se obtiene como:
    \begin{equation}
        t_{\textit{insert\_validation}} = t_{\textit{insert\_molecule\_3d}} - t_{\textit{fft\_correlation}}
    \end{equation}

    El experimento se realizó sobre un volumen de $500\times500\times250$ vóxeles con la proteína \textit{2uv8\_10A.pns}, alcanzando un grado de ocupancia del \textbf{32\%} con \textbf{2\,552 monómeros insertados} en un tiempo total de \textbf{191.84\,s}. La Figura~\ref{fig:profile-breakdown} muestra la curva de ocupancia frente al tiempo, con el área bajo la curva coloreada según la etapa dominante en cada tramo.

    \begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{archivos/profile_occupancy_breakdown.png}
        \caption{Desglose del tiempo de ejecución por etapa de la implementación adaptada del algoritmo Tetris. El área coloreada bajo la curva de ocupancia indica el tiempo acumulado de cada operación (experimento: volumen $500\times500\times250$, proteína \textit{2uv8\_10A.pns}, 32\% de ocupancia, 2\,552 monómeros).}
        \label{fig:profile-breakdown}
    \end{figure}

    Los resultados, resumidos en la Tabla~\ref{tab:profile}, revelan que el cuello de botella principal es la función \texttt{pick\_seed} (\textbf{62.1\%}), seguida por la correlación FFT 3D (\textbf{14.1\%}) y la validación e inserción (\textbf{12.0\%}), que juntas componen el método \texttt{insert\_molecule\_3d}. Las operaciones de generación de \textit{in-shell} y rotación afín resultan prácticamente despreciables ($<2\%$ combinadas).
    ...
    \hline
    \end{tabular}
    \caption{Desglose del tiempo de ejecución de la implementación adaptada por operación (ejecución en CPU, volumen $500\times500\times250$, proteína \textit{2uv8\_10A.pns}, 32\% de ocupancia).}
    \label{tab:profile}
    \end{table}

    Estos datos orientan la estrategia de optimización en dos direcciones: (1) la aceleración GPU (Fase~4) reduce el coste de la correlación FFT y la validación mediante paralelismo masivo; (2) la función \texttt{pick\_seed} se beneficia igualmente del backend \texttt{CuPy}, que ejecuta el \texttt{argwhere} sobre el volumen completo en paralelo, eliminando el principal cuello de botella identificado.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Identificación de operaciones paralelizables}: Las dos operaciones más costosas del algoritmo son \texttt{insert\_molecule\_3d} —método integrado que engloba la correlación FFT 3D y la validación de inserción, representando conjuntamente el 46.1\% del tiempo total— y \texttt{pick\_seed}, con un 34.1\%, cuyo \texttt{argwhere} sobre el volumen completo se beneficia directamente del paralelismo masivo de la GPU. Ambas operaciones cuentan con implementaciones optimizadas en \texttt{CuPy}. Las operaciones de rotación afín y filtrado gaussiano son computacionalmente despreciables ($<2\%$) pero también se redirigen al backend GPU para mantener los datos en memoria de dispositivo y evitar transferencias innecesarias.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.85\textwidth]{archivos/esquema_gpu_cpu.png}
        \caption{Esquema de enrutamiento CPU/GPU. En función de la disponibilidad de una GPU compatible con CUDA, las operaciones de correlación FFT, filtrado gaussiano, rotación afín y muestreo de semilla se ejecutan sobre \texttt{CuPy} (GPU) o \texttt{SciPy/NumPy} (CPU), manteniendo la misma lógica algorítmica en ambos casos.}
    ...
    Otros (Python, I/O)                        &  20.16 & 10.5\% & 35.06 & 44.4\% \\
    ...
    \hline
    \end{tabular}
    \caption{Comparación del desglose de tiempo de ejecución entre las implementaciones CPU y GPU (volumen $500\times500\times250$, proteína \textit{2uv8\_10A.pns}, 32\% de ocupancia). La GPU fue una NVIDIA RTX 4090.}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Scripts de benchmark} (\texttt{benchmark\_comparison.py}, \texttt{benchmark\_scientific\_comparison.py}): ejecutan ambos algoritmos y registran las métricas clave.
        \item \textbf{Generador de gráficas} (\texttt{plot\_comparison.py}): produce las figuras comparativas presentadas en el Capítulo~\ref{cap:resultados}.
        \item \textbf{Tests de validación}: % TODO: pendientes de implementar
    \end{itemize}

    \item \textbf{Métricas de evaluación}: Las variables dependientes registradas en cada experimento son: tiempo de ejecución ($t$), ocupancia de proteínas ($\rho_{prot}$), ocupancia total ($\rho_{total}$), throughput (monómeros/segundo) y speedup ($t_{SAWLC}/t_{Tetris}$).
\end{enumerate}
